...
function QuestionPane(ques, raw) {

    this.ques = ques;
};

QuestionPane.prototype.container = function () {

    if (!this.$ctr) {

        this.$ctr =
            $('<div class="col-md-12" />')
                .append($('<div class="title">' + this.ques + '</div><div class="row"></div>'));

        this.$row = this.$ctr.find(".row");

        this.$pie = $('<div class="col-md-4 chart"><canvas width="300" height="300"></canvas></div>')
            .appendTo(this.$row);

        this.$bar = $('<div class="col-md-8 chart"><canvas height="300"></canvas></div>')
            .appendTo(this.$row);

        this.$ctr = $('<div class="row" />').append(this.$ctr);
        this.$pie = this.$pie.find("canvas");
        this.$bar = this.$bar.find("canvas");
    }

    return this.$ctr;
};

QuestionPane.prototype.render = function (filter, raw) {

    var cf = crossfilter(raw);

    for (var key in filter) {
        if (filter.hasOwnProperty(key)) {

            cf = cf.dimension(function (d) { return d[key] }).filter(filter[key]);
        }
    }

    var data = cf.dimension(function (d) { return d.response; }).group().all();
    ...
    for (var i = 0; i < data.length; i++) {
        op.labels.push(data[i].key);
        op.datasets[0].data.push(data[i].value);
        op.datasets[0].backgroundColor.push(getColor(data[i].key).background);
        op.datasets[0].borderColor.push(getColor(data[i].key).border);
    }

    this.pieChart = new Chart(this.$pie, {
        type: 'pie',
        data: op,
        options: {}
    });
    ...
    var dates = crossfilter(raw).dimension(function (d) { return JSON.stringify(d.Timestamp); }).group().all();

    var data = cf.dimension(function (d) { return JSON.stringify({ Gender: d.Gender.trim(), ts: d.Timestamp }); }).group().reduce(
        function (a, d) {
            a += d.score;
            return a;
        },
    ...
    for (var i = 0; i < dates.length; i++) {
        var date = dates[i].key.replace(/\"/g, "");
        dj[date] = { idx: i, m: null, f: null };
    }

    for (var i = 0; i < data.length; i++) {

        var date = data[i].ts, date = date.replace(/\"/g, ""), idx = dj[date].idx;

        if (data[i].Gender == "Male") {
            dj[date].m = data[i].value;
        } else {
            dj[date].f = data[i].value;
        }
    }

    op.labels = Object.keys(dj);

    for (var i = 0; i < op.labels.length; i++) {

        op.datasets[0].data[i] = null;
        op.datasets[1].data[i] = null;

        op.datasets[0].data[dj[op.labels[i]].idx] = dj[op.labels[i]].m;
        op.datasets[1].data[dj[op.labels[i]].idx] = dj[op.labels[i]].f;
    }


    this.barChart = new Chart(this.$bar, {
        type: 'bar',
        data: op,
        options: {
            scales: {
