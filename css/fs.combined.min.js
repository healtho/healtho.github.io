...
QuestionPane.prototype.render = function (filter, raw) {
    ...
    var data = cf.dimension(function(d) { return JSON.stringify({ Gender: d.Gender.trim(), ts: d.Timestamp }); }).group().all();
    ...
    for (var i=0; i<op.labels.length; i++) {
        ...
        op.datasets[0][dj[op.labels[i]].idx] = dj[op.labels[i]].m;
        op.datasets[1][dj[op.labels[i]].idx] = dj[op.labels[i]].f;
    }
